...
final class DefaultBotHandlers {
    ...
    /// Обрабатывает обновления в чате
...
//            try await connection.bot.sendMessage(params: params)
            ...
            let params = TGSendPhotoParams(chatId: .chat(userId), photo: .url("https://disk.yandex.ru/i/q8WgWkEpRlF7xQ"), caption: welcomeMessage, replyMarkup: .inlineKeyboardMarkup(keyboard))
            
            /// Отправка сообщения с изображением и кнопками
            do {
                try await connection.bot.sendPhoto(params: params)
            } catch {
                print("Не удалось отправить сообщение с изображением и кнопками: \(error)")
            }
        })
    }
    
    /// Обработывает сообщений
    ...
    private static func commandShowButtonsHandler(app: Vapor.Application, connection: TGConnectionPrtcl) async {
            await connection.dispatcher.add(TGCommandHandler(commands: ["/show_buttons"]) { update, bot in
                ...
                let keyboard: TGInlineKeyboardMarkup = .init(inlineKeyboard: buttons)
                let params: TGSendMessageParams = .init(chatId: .chat(userId), text: "Keyboard active", replyMarkup: .inlineKeyboardMarkup(keyboard))
                try await connection.bot.sendMessage(params: params)
            })
        }
    ...
    private static func buttonsActionHandler(app: Vapor.Application, connection: TGConnectionPrtcl) async {
        ...
        await connection.dispatcher.add(TGCallbackQueryHandler(pattern: "/web_site") { update, bot in
            ...
            let params = TGSendMessageParams(chatId: .chat(callbackQuery.message?.chat.id ?? 0), text: "Привет! Вот ссылка на наш сайт:", replyMarkup: .inlineKeyboardMarkup(keyboardMarkup))
            
            do {
                try await bot.sendMessage(params: params)
            } catch {
                print("Не удалось отправить сообщение со ссылкой на сайт: \(error)")
            }
        })
...
"""
            ...
            do {
                ...
                print("Не удалось отправить сообщение с кнопками: \(error)")
            }
        })
        
        
//            await connection.dispatcher.add(TGCallbackQueryHandler(pattern: "press 1") { update, bot in
//                let params: TGAnswerCallbackQueryParams = .init(callbackQueryId: update.callbackQuery?.id ?? "0", text: update.callbackQuery?.data  ?? "data not exist", showAlert: nil, url: nil, cacheTime: nil)
